...
public class Solution {
    ...
    public static void cleanAllApartments(List<Apartment> apartments) {
        for(Apartment now:apartments){
            if (now instanceof Apt1Room){
                ((Apt1Room) now).clean1Room();
            }
            else if (now instanceof Apt2Room){
                ((Apt2Room) now).clean2Rooms();
            }
            else if (now instanceof Apt3Room){
                ((Apt3Room) now).clean3Rooms();
            }
        }
    }
